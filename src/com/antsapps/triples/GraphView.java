...
public class GraphView extends View {
  ...
  private void drawPlot(Canvas canvas) {
    long[][] rawData = mStatistics.getData();
    float[] scaledData = new float[rawData.length * 2];
    for(int i = 0; i < rawData.length; i++) {
//      scaledData[2*i] = getGraphXOffset() + i * getGraphWidth() / rawData.length;
      scaledData[2*i] = convertDateToXCoord(rawData[i][0]);
      scaledData[2*i + 1] = convertTimeToYCoord(rawData[i][1]);
    }
